 ...
 protected:
  void SetUp() {
    ...
    schema_ = arrow::schema({f_bool, f_int8, f_int16, f_int32, 
        f_int64, f_float,  f_double, f_binary});
    // schema_ = arrow::schema({f_string, f_string, f_string, f_string});
    // schema_ = arrow::schema({f_bool, f_bool, f_bool, f_bool});
     ...
     std::shared_ptr<UnsafeRowWriterAndReader> unsafe_row_writer_reader = 
      ...
      while(unsafe_row_writer_reader->HasNext()) {
        ...
        auto data = buffer->data();
         std::cout << "the length is " << length  << " and the buffer size is " << buffer->size() << " and the buffer data address is " << &data<< "\n";
        std::cout << "the 0 and 1 value is " << "\n" << std::flush;
